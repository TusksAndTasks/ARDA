...
function LeadExchange() {
  return (
    <div className="ml-3">
      ...
      <div className="mr-2 flex max-w-[500px] flex-col items-center rounded-b-xl bg-bronze p-2 pb-0 text-center">
        <TypographyPrimitive as="p" fontSize={fontSizes.LARGE} font={Fonts.GENERALMEDIUM}>
          {LeadExchangeData.communityRules}
        </TypographyPrimitive>
      ...
      <div className="mr-2 mb-2 flex max-w-[700px] flex-wrap justify-center gap-5 rounded-b-lg bg-bronze px-2 pb-5">
        {LeadExchangeData.linksSection.links.map((linksBlock) => (
          <div
            key={linksBlock.theme}
            className="h-[200px] w-[200px] rounded-b-lg bg-lightgold py-3 text-center"
          >
            <TypographyPrimitive as="p" fontSize={fontSizes.LARGEPLUS} font={Fonts.GENERALMEDIUM}>
              {linksBlock.theme}
            </TypographyPrimitive>
            {linksBlock.thematicLinks.map((link) => (
              <div key={link} className="m-1 rounded-b-lg bg-black pb-3 transition-all hover:pb-6">
